<template>
	<div class="main">
		<div class="header">
			...
			<div class="close" @click="closeOpt"><i class="icon iconcuo"></i></div>
		</div>
			...
			addCart() {
				...
				if(this.token != 0) {
					this.$http.post('addCart', {
						...
						productId: this.info_data[0].product_id,
						num: this.p_number
					}).then((res) => {
	...
	.main {
		...
		.btns {
			...
			.btn1 {
				flex: 1;
				height: 100%;
				background: -webkit-linear-gradient(138deg, #ffa600, #ffb000 77%, #ffbc00);
